<template>
	<view>
		...
		<!-- 底部菜单 -->
		<view class="footer" v-if="user_type!=1">
			<view class="icons">
			...
			<view class="btn">
				<view class="joinCart" @tap="joinCart">联系店铺</view>
				<view class="buy" @tap="buy">竞价</view>
			</view>
		</view>
		<!-- share弹窗 -->
			...
			<view class="layer" @tap.stop="discard">
				<view class="content">
					<view class="popup_title">
						...
						<view class="close" @tap="hideSpec">x</view>
					</view>
					<!-- <view class="list_contact">
						<view class="img_list">
							<image src="/static/img/share/wx.png" mode=""></image>
						</view>
					...
					<!-- <view class="list_contact">
						<view class="img_list">
							<image src="/static/img/share/qq.png" mode=""></image>
						</view>
						<view class="list_text">
							152525235
						</view>
					...
					<view class="list_contact">
						<view class="img_list">
							<image src="/static/img/share/mail.png" mode=""></image>
						</view>
						<view class="list_text">
							{{goodsData.email}}
						</view>
		...
		<view class="swiper-box">
			<swiper circular="true" autoplay="true" @change="swiperChange">
				<swiper-item v-for="swiper in swiperList" :key="swiper.id">
					<image :src="'http://192.168.101.52/'+swiper.url"></image>
				</swiper-item>
			</swiper>
		...
		<view class="info-box goods-info">
			...
			<view class="price">${{goodsData.bounty}}</view>
			<uni-steps :options="list1" :active="active" v-if="showStatus"/>
		</view>
		
		<!-- 评价 -->
		<view class="info-box comments" id="comments">
			<view class="row">
				<view class="text">用户报价({{offer.length}})</view>
				<view class="arrow" @tap="toRatings">
					
				</view>
			</view>
			<view class="comment"  v-if="offer.length>0">
				<view class="user-info" >
					<view style="display: flex;">
						<view class="face" v-if="offer[0].avatar"><image :src="offer[0].avatar"></image></view>
						<view class="face" v-else><image src="../../static/img/face.jpg"></image></view>
						<view class="username">{{offer[0].nickname}}</view>
					</view>
					<view class="dateText">
						{{offer[0].created_at}}
					</view>
			...
			<view class="nobody" v-else>
				空
			</view>
			<view class="show" @tap="toRatings" v-if="offer.length>0">
				— 查看更多 —
			</view>
		</view>
		<!-- 详情 -->
		<view class="description">
			...
			<view class="content">
				...
				<view ref="richtext" v-for="(item,index) in descriptionStr" :key="index" >
...
export default {
	...
	onLoad(option) {
		...
		if(!option.good_id){
			this.$http('/api/shop/goodContent',{id:option.good_id}).then(res=>{
				this.showStatus=false
			})
			
		}else{
			this.$http('/newtasks/taskindex',{id:37}).then(res=>{
				for(var i=0;i<res.data.data.attatchment.length;i++){
					res.data.data.attatchment[i].url=res.data.data.attatchment[i].url.replace(/\\/g,"/");
				}
				this.swiperList=res.data.data.attatchment
				this.descriptionStr=JSON.parse(res.data.data.detail.desc)
	...
	methods: {
		...
		callPhone(){
			uni.makePhoneCall({
			    phoneNumber: this.goodsData.phone 
			});
		},
		// 客服
		toChat(){
		...
		keep(){
			...
			if(this.isKeep){
				this.$http("/api/user/addFocus",{task_id:''},"get").then(res=>{
					
				})
			}else{
				this.$http("/api/user/deleteFocus",{id:''}).then(res=>{
		...
		buy(){
			...
			// }
			uni.navigateTo({
				url:'../order/bidding?id='+this.goodsData.id+"&price="+this.goodsData.bounty
			})
			// this.toConfirmation();
		},
		//商品评论
		toRatings(){
			uni.navigateTo({
				url:'ratings/ratings?task_id='+ this.goodsData.id
			})
		},
		//跳转确认订单页面
		toConfirmation(){
