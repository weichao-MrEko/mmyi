<template>
	<view>
		...
		<view class="form re">
			<view class="username">
				<input placeholder="请输入用户名" v-model="username" placeholder-style="color: rgba(255,255,255,0.8);" @blur="usercode"/>
				 <icon type="success" size="16" color="#04ff00" v-if="icon"/>
				 <view class="iconInfo">
				 	{{iconText}}
				 </view>
			</view>
			...
			<view class="code">
				<input placeholder="请输入邮箱" type="text" v-model="mail" placeholder-style="color: rgba(255,255,255,0.8);"/>
		...
		methods: {
			usercode(){
				...
				if(reg.test(this.username)){
					this.$http("/newapi/checkUserName",
					{param:this.username})
						.then(res=>{
							console.log(res)
							if(res.data.status=='y'){
								// this.icon=true
								this.iconText=res.data.info
							}else{
								// this.icon=false
								this.iconText=res.data.info
					...
					this.username=''
					uni.showToast({title:"请输入英文和数字",icon:"none"});
				}
